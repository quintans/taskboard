...
var EVENT_LOGIN_OK = "event:loginOk";
...
;
function AppCtrl($scope, $location, $window, taskBoardService, $localStorage, $http, $interval) {
    var refreshToken;
    // initialize form
    var showLogin = function (visible) {
        ...
        if (!visible) {
            refreshToken = $interval(function () {
                ...
                $http.post("refresh", "", config).success(function (data) {
                    if (data !== "") {
                    ...
                    // Erase the token if the user fails to refresh
                    delete $localStorage.jwtToken;
                });
            }, 600000); // 10 minutos
    ...
    $scope.$on(EVENT_LOGIN_OK, function (event, newPrincipal) {
        showLogin(false);
        $scope.loggedIn = true;
    });
    /*
